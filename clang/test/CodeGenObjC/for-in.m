...
void t0() {
  ...
  for (NSString *i in array) {	// expected-warning {{collection expression type 'NSArray *' may not respond}}
    ...
    // CHECK-NEXT: [[isCls:%.*]] = call noundef zeroext i1 bitcast {{.*}}@objc_msgSend to i1 (i8*, i8*, {{.*}})(i8* noundef [[theItem:%.*]], i8* noundef [[kindOfClassSel]], i8* noundef [[expectedClsI8]]), !nosanitize
    // CHECK: br i1 [[isCls]]
