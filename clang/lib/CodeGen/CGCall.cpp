...
void CodeGenModule::ConstructAttributeList(
  ...
  if (RetTyPtr->isRecordType()) {
    ...
    RetPartialInit =
        RetLayout.isPartialInit() || (RetAI.getPaddingType() != nullptr);
  }

  // If we're coercing to a type of different size, we're introducing more
  // padding bits
  if (RetAI.canHaveCoerceToType()) {
    const size_t RetRealSize =
        getDataLayout().getTypeSizeInBits(getTypes().ConvertType(RetTy));
    const size_t RetLowSize =
        getDataLayout().getTypeSizeInBits(RetAI.getCoerceToType());
  ...
  for (CGFunctionInfo::const_arg_iterator I = FI.arg_begin(),
    ...
    if (AI.canHaveCoerceToType()) {
      const size_t ArgRealSize =
          getDataLayout().getTypeSizeInBits(getTypes().ConvertType(ParamType));
      const size_t ArgLowSize =
          getDataLayout().getTypeSizeInBits(AI.getCoerceToType());
