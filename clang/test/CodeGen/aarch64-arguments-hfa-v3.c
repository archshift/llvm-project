...
// CHECK: define partialinit %struct.HFAv3 @test([4 x <4 x float>] partialinit %{{.*}}, [4 x <4 x float>] partialinit %{{.*}}, [4 x <4 x float>] partialinit %{{.*}})
HFAv3 test(HFAv3 a0, HFAv3 a1, HFAv3 a2) {
  return a2;
}

// CHECK: define partialinit %struct.MixedHFAv3 @test_mixed([4 x <4 x float>] partialinit %{{.*}}, [4 x <4 x float>] partialinit %{{.*}}, [4 x <4 x float>] partialinit %{{.*}})
